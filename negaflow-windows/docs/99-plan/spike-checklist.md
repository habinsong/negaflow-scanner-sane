...
기준일: 2026-08-04 (결과 반영 2026-08-06)
상태: 차단 항목은 모두 닫혔다 — 아래 표 참조
목적: 문서 전체에 흩어진 미확인 항목을 한 곳에 모아 실행 순서를 정한다

## 지금까지 닫힌 것 (2026-08-06, OpticFilm 8100 실기)

한 줄 요약: **드라이버를 바꾸지 않고 스캔이 된다.** 각 항목의 전문은 소유
문서에 있다.

| ID | 결과 |
| --- | --- |
| S-1 | **통과.** `genesys:usbscan:000` 열거·옵션 덤프·600~7200 dpi 스캔 |
| S-2 | 실패 → `002-binary-output-mode.patch` 로 해결 |
| S-3 | **폐기.** A 경로가 통과해 WSL2 가 필요 없다 |
| S-4 | 장치명이 `:libusb:` 가 아니라 `:usbscan:` 다. 판정 코드를 고쳤다 |
| S-5 | 주소가 **바뀌지 않는다.** macOS 의 churn 이 없다 |
| S-6 | **통과.** 한국어 Windows 에서 `LC_ALL` 없이도 영어로 나온다 |
| E-1 | **닫힘.** `HAVE_DLOPEN` 과 `cygsane-` 이름이 둘 다 필요했다 |
| E-2 | **honor 한다.** 단 `.conf` 를 그 디렉터리에 직접 둬야 한다 |
| C-1 | **통과.** 단 `scanimage` 에 `007` 패치가 필요했다 |
| C-2 | **복구 절차가 없다.** 강제 종료하면 전원을 껐다 켜야 한다 |
| C-3 | graceful 6,890 ms. 이것이 유예 15,000 ms 의 근거다 |
| U-5 / DC-3 | **동시 열기가 실패하지 않는다.** `usbscan.sys` 가 배타 접근을 강제하지 않아 어댑터가 뮤텍스로 막는다 |
| D-09 | **폐기.** 드라이버 바인딩 자체를 하지 않는다 |

여기서 배운 것 하나를 남긴다. **판정 기준을 먼저 정하고 시작한다.** 취소
문제에서 "신호가 전달되는가"로 판정했다가 두 번 틀렸다. 옳은 기준은 "취소
뒤에 다음 스캔이 되는가" 였다.
