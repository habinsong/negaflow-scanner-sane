...
#### 5.1a 실측 결과 — 2초도 부족했다 (2026-08-06)

위 예측이 맞았다. 다만 자릿수가 달랐다.

```text
OpticFilm 8100, 스캔 6초 지점에서 CTRL_BREAK
  scanimage 종료까지        6,890 ms
```

2초로는 그 한가운데에서 죽인다. 그래서 `kCancelGracePeriod` 를 **15초**로
잡았다.

**같은 값을 두 곳이 써야 한다.** 취소 경로만 고치고 획득 감시견을 두고
있었는데, 감시견 쪽은 macOS 를 그대로 옮긴 0.5초였다.

```cpp
// child.cpp — 고치기 전
watchdog.start(firstProgressTimeout, stallTimeout,
               [&child] { terminateChild(child, kTimeoutGraceMs); });   // 500 ms
```

같은 신호(CTRL_BREAK)를 같은 프로세스에 보내니 같은 시간이 걸린다. 감시견이
180초 무진행으로 깨어나 0.5초 뒤 `TerminateJobObject` 를 부르면, 그것이 곧
C-2 가 말하는 "스캔 도중 강제 종료" 이고 스캐너는 전원을 다시 넣어야 한다.
지금은 `kAcquisitionGraceMs = kCancelGracePeriod` 다.

유틸리티 호출(`-L`, `-A`)의 0.5초는 그대로 둔다. 스캔하지 않으므로 빨리
죽여도 잃을 것이 없다.

**맥과 갈라지는 것은 알고 그러는 것이다.** 맥은 libusb 로 열어서 죽여도
다음 스캔이 되고, Windows 는 usbscan.sys 로 열어서 죽이면 물린다. 이 차이는
이번에 Windows 에서만 관측한 것이다(C-2).

남는 위험을 정직하게 적는다. 감시견이 깨어났다는 것은 180초 동안 진행이
없었다는 뜻이고, 그때 `scanimage` 는 드라이버 호출 안에서 막혀 있을 수 있다.
그러면 CTRL_BREAK 를 15초 기다려도 안 끝나고 강제 종료로 간다. **감시견이
발동한 뒤에는 전원 재인가가 필요할 수 있다.** 15초는 "응답할 수 있는 상태면
살린다" 는 값이지 만능이 아니다. 감시견 발동을 실기로 재현하지는 않았다 —
재현하려면 스캐너를 일부러 물려야 한다.
...
#### 결과 — **통과, 단 `scanimage` 를 고쳐야 했다** (2026-08-06, OpticFilm 8100)

| 항목 | 결과 |
| --- | --- |
| 1~3. `CTRL_BREAK` 로 종료하는가 | 한다. **6,890 ms** — 진행 중인 `sane_read` 가 끝나고 헤드가 홈으로 가는 시간이다 |
| 4. `CTRL_C_EVENT` | 쓰지 않는다. 특정 그룹에 보낼 수 없다(§4.1) |
| 5. 종료 후 같은 장치를 다시 열 수 있는가 | **된다.** 취소 뒤 스캔이 17초에 3,030,277 바이트 |
| 6. 콘솔 없는 부모에서도 되는가 | 그대로는 안 된다. 숨긴 콘솔을 확보해야 한다(§4.1) |

**단, 손대지 않은 `scanimage` 로는 실패한다.** 두 겹의 문제가 있었고 둘 다
`sane-runtime/patches/007-cancel-on-sigbreak.patch` 로 고쳤다 — §4.1a 참조.
그 패치 없이 `CTRL_BREAK` 를 보내면 기본 핸들러가 전송 도중 프로세스를 끝내고,
스캐너는 전원을 다시 넣기 전까지 돌아오지 않는다.

**판정을 종료 코드로 하면 안 된다.** 처음에 그렇게 해서 두 번 틀렸다. 기준은
"취소 뒤에 다음 스캔이 되는가" 하나다.
...
#### 결과 — **복구 절차가 없다** (2026-08-06, OpticFilm 8100)

```text
2. 강제 종료 직후 scanimage -f      장치가 보인다 (열거는 전송을 안 한다)
3. 즉시 scanimage -A -d <dev>       "Error during device I/O", 그 뒤로는
                                     "open of device ... failed: Invalid argument"
4. 몇 초 뒤에도 안 된다              20분 뒤에도 안 됐다
5. 물리적 재연결이 필요한가          **스캐너 전원을 껐다 켜야 한다**
```

전원을 껐다 켜는 것 말고는 방법이 없다. 사용자 모드 복구 경로를 전부
확인했다 — `IOCTL_RESET_PIPE`/`IOCTL_CANCEL_IO` 는 모든 조합이 무효,
`CM_Query_And_Remove_SubTree` 와 `CM_Setup_DevNode` 는 `CR_ACCESS_DENIED`,
`IOCTL_USB_HUB_CYCLE_PORT` 는 최신 Windows 에서 제거됐다.

**그래서 §5.2 의 "복구 절차"는 존재할 수 없다.** 강제 종료를 **하지 않는
것**이 유일한 대책이고, 그것이 §4.1a 의 패치와 `kCancelGracePeriod` 15,000 ms
가 존재하는 이유다. 유예 안에 끝나지 않아 3단계로 넘어가면 그 스캐너는
사용자가 전원을 껐다 켤 때까지 죽는다 — 그 사실을 여기 적어 둔다.
...
#### 결과 — genesys / 600 dpi (2026-08-06, OpticFilm 8100)

```text
graceful   취소 요청 → 종료        6,890 ms
           종료 → 다음 스캔 성공    바로 된다 (17초 스캔이 그대로 성공)
강제       취소 요청 → 종료        즉시
           종료 → 다음 스캔 성공    **되지 않는다.** 전원을 껐다 켜야 한다 (C-2)
```

6,890 ms 가 `kCancelGracePeriod` 를 15,000 ms 로 정한 근거다. 다른 백엔드와
해상도는 아직 측정하지 않았다. 다만 이 값은 "취소가 느린 것"과 "스캐너가
죽는 것" 사이의 선택이므로, 모르는 쪽으로는 넉넉하게 두는 것이 맞다.
...
#### 결과 — 3번 통과 (2026-08-06, 가상 scanimage)

중간 TIFF 는 `SampleCleanup` 이라는 RAII 가드가 지운다. 취소로 조기 반환해도
소멸자가 돌기 때문에 경로가 하나뿐이다. 실제로 확인했다 — 다중 노출 스캔을
2번째 패스 중에 취소한 뒤 작업 디렉터리에 `.negaflow-sample*.tiff` 가 하나도
남지 않았다.

```text
{"type":"error", …, "message":"cancelled: 스캔이 취소되었습니다."}
종료 코드 1, 15,000 ms
남은 중간 파일: 없음
```

15,000 ms 는 가상 픽스처가 신호를 무시해서다(`stall` 시나리오에 핸들러가
없다). 응답 없는 자식에 대한 예상 경로이고, `kCancelGracePeriod` 가 실제로
지켜지는 것도 여기서 함께 보인다.

**RAII 로는 못 막는 경우가 하나 있다.** 어댑터 자신이 강제 종료되면 소멸자가
돌지 않아 중간 파일이 남는다. 호스트가 우리를 죽이는 경우이므로 우리 쪽에서
할 수 있는 일이 없다. 다음 스캔이 같은 이름을 덮어쓰므로 무한히 쌓이지는
않는다.

4번(다음 스캔이 정상 동작하는가)은 C-1 이 이미 답했다 — graceful 취소 뒤
다음 스캔은 바로 된다.
