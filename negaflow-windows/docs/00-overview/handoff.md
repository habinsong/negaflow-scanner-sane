...
기준일: 2026-08-06 (12차 갱신 — **실기로 스캔이 된다.** 드라이버를 바꾸지 않고)
기준 커밋: 2b4477f
...
Plustek OpticFilm 8100 을 Windows 에서 **드라이버 교체·관리자 권한·재부팅
없이** 스캔한다. 제조사 드라이버(`usbscan.sys`)를 그대로 두고 그것이 열어주는
raw USB 통로를 쓴다 — `sanei_usb` 에 백엔드를 하나 더 붙였다. SilverFast 와
VueScan 이 계속 동작한다. 600~7200 dpi 전부, 연속 스캔 8/8, 취소해도 스캐너가
살아남는다. Zadig 는 필요 없고, 애초에 WinUSB 로 바꿨어도 같은 벽에 부딪혔을
것이다(libusb 의 Windows 백엔드에는 장치 리셋이 없다).

남은 차단 항목은 **D-1(코드 서명 인증서)** 하나이고, 그것은 기술 문제가 아니다.

> **2026-08-14 정정 — 위 문단은 개발 기기에서만 참이었다.**
>
> OpticFilm 8100 이 아무것도 깔지 않고 열린 이유는 **SilverFast 가 깔아 둔
> `oem113.inf`** 가 그 장치를 이미 `usbscan` 서비스에 묶어 놓았기 때문이다. 장치를
> usbscan.sys 에 묶는 일은 INF 만 할 수 있고, 벤더 소프트웨어가 없는 기기에서는
> OpticFilm 도 Epson Perfection V700 과 똑같이 `CM_PROB_FAILED_INSTALL`(코드 28) 로
> 남는다. 그 상태에서 SANE 은 "스캐너 없음" 만 말한다 — 코드 문제처럼 보이지만 아니다.
>
> 그래서 `tools/usbscan-bind/negaflow-usbscan.inf` 가 배포물에 들어간다. 바이너리는
> 하나도 설치하지 않고 `Include=sti.inf` / `Needs=STI.USBSection` 으로 Windows 가 이미
> 갖고 있는 usbscan.sys 를 가리키기만 한다. 벤더 스택을 밀어내지 않는다.
>
> 다만 **INF 설치에는 승격이 필요하고, 서명 강제를 통과하려면 카탈로그 서명이 필요하다.**
> 즉 D-1 은 실행 파일만의 문제가 아니라 **이 INF 의 문제이기도 하다.** 이것이 "플러그인만
> 깔면 스캔된다" 를 가르는 지점이다.
...
**실기 검증은 끝났다** (2026-08-06). C 항목은 D-1 만 남았다.

```text
A. 병합 입력 스트리밍       7200 dpi 12 패스면 입력만 13.2 GB   §3.3
B. imaging/tiff 읽기 경로   핸들 기반 검증 (쓰기 경로는 되어 있다)  §3.4
C. 실기 검증                S-1·S-2·C-1~C-4 **닫힘**. D-1(서명)만 남음
D. SANE 런타임 배포          scanimage.exe 를 어떻게 담을 것인가    §3.6
```

### 3.0a 실기로 확정된 것 (2026-08-06, OpticFilm 8100)

```text
드라이버 교체 없이 스캔      usbscan.sys 로 연다. Zadig 불필요
해상도 600~7200 dpi          전부 통과. 7200 에서 필름 그레인이 분해된다
연속 스캔                    8/8. RESET_PIPE 를 걸지 않는 것이 핵심이었다
취소                         SIGBREAK + 플래그 방식. 6,890 ms, 스캐너 생존
동시 스캔 차단               usbscan.sys 가 안 막아서 어댑터가 뮤텍스로 막는다
Epson 인자 계약              17검사로 고정 (하드웨어 없이)
```

**세 가지를 조심한다.**

```text
Gray 모드를 돌리지 않는다     upstream genesys 결함으로 무한 정지한다
스캔 중 강제 종료하지 않는다   스캐너가 죽고 전원 재인가 외에 복구가 없다
