...
IR 패스에는 `--film-type` 과 `--color-correction` 을 보내지 않는다. 그림 쪽
제어라 적외선 채널에 의미가 없다.

**`--gamma-correction` 은 예외이며 IR 패스에도 보낸다**(2026-08-11, macOS 3d9c3d3).
장치 기본 감마로 IR 을 찍으면 GT-X900 실측에서 신호가 흰쪽 끝에 몰린다 — 프레임의
2~3.4% 가 65535 에 잘리고 21~55% 가 65000 위에 뭉친다. 결함 신호는 필름 베이스 바로
아래에 있으므로 그 구간이 통째로 압축·절단된다. 같은 자리를 선형 테이블로 찍으면 필름
베이스가 0.81 에 앉고 잘림이 없다. 초점도 같은 이유로 두 패스가 같아야 한다 — 초점면이
다르면 배율이 달라져 결함 지도가 결함에서 몇 px 옆에 놓인다.

이 문단은 한동안 감마도 보내지 않는다고 적고 있었다. **그 서술이 틀렸다.**
