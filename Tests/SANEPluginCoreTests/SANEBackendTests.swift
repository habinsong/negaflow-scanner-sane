...
final class SANEBackendCapabilityTests: XCTestCase {
    ...
    /// 모드를 세우기 전 덤프에서는 gamma-correction이 비활성으로 나올 수 있다. 그때 옵션을
    /// 건너뛰면 장치 기본 감마로 스캔해 놓고 호스트가 선형 raw로 읽어 색이 어긋난다.
    /// 인자는 --mode 뒤에 실리므로 같은 호출 안에서 활성화된다.
    func testEpson2AppliesLinearGammaEvenWhenTheOptionIsReportedInactive() throws {
        let dump = """
        --mode Color|Gray [Color]
        --source Flatbed|TPU8x10 [TPU8x10]
        --depth 8|16 [16]
        --resolution 3600dpi [3600]
        --gamma-correction Default|User defined|High density printing [inactive]
        --color-correction None|Built in CCT profile [None]
        -l 0..215.9mm (in steps of 0.1) [0]
        -t 0..297.1mm (in steps of 0.1) [0]
        -x 1..203.2mm (in steps of 0.1) [203.2]
        -y 1..254mm (in steps of 0.1) [254]
        ...
        XCTAssertEqual(argValue(args, "--gamma-correction"), "User defined")
        let modeIndex = try XCTUnwrap(args.firstIndex(of: "--mode"))
        let gammaIndex = try XCTUnwrap(args.firstIndex(of: "--gamma-correction"))
        XCTAssertLessThan(modeIndex, gammaIndex, "감마는 모드를 세운 뒤에 실려야 적용된다.")
    }

    /// 선형으로 세울 값이 없으면 비선형 스캔을 선형 raw로 넘기지 않고 그 자리에서 멈춘다.
    func testEpson2FailsClosedWhenNoLinearGammaValueExists() {
        let dump = """
        --mode Color [Color]
        --source TPU8x10 [TPU8x10]
        --depth 16 [16]
        --resolution 3600dpi [3600]
        --gamma-correction Default|High density printing [Default]
        --color-correction None [None]
        -l 0..215.9mm [0]
        -t 0..297.1mm [0]
        -x 1..203.2mm [203.2]
        -y 1..254mm [254]
        ...
        options.scanArea = ScanArea(widthMM: 36, heightMM: 24)
        let media = SANEBackend.resolveMedia(dump: dump, options: options)
        XCTAssertNil(media.gammaCorrection)
        XCTAssertThrowsError(try SANEBackend.validateExactOptions(options, media: media))
    }
