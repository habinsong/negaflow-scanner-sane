...
extension SANEBackend {
    ...
    static func resolveMedia(
           ...
           source == nil || source.map({ isTransparencySource($0) }) == true {
            if filmTypeOptionName == "negative",
               backend == "coolscan2" || backend == "coolscan3" {
                ...
                //
                // 필요한 것은 극성뿐이고 그것은 `polarityMatches` 가 이미 걸렀다. 두 갈래를
                // 하나로 둔다. "slide" 밖에 없는 장치는 아래 fallback 이 집는다.
        ...
        return MediaSelection(
            ...
            hasAutofocusOption: opts.isActive("autofocus"),
            ...
            focusRange: opts.isActive("focus") ? opts.numericRange("focus") : nil,
