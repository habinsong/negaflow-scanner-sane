...
SOURCE_ARCHIVE="$(find "$EXPANDED" -path '*negaflowScannerSetup.pkg/Scripts/sane/negaflow-sane-*-source.tar.gz' -print -quit)"
...
INSTALLED_SOURCE="$(find "$INSTALLED" -maxdepth 1 -name 'negaflow-sane-*-source.tar.gz' -print -quit)"
...
DMG_SOURCE="$(find "$TEMPORARY/mount" -maxdepth 1 -name 'negaflow-sane-*-source.tar.gz' -print -quit)"
test -s "$DMG_SOURCE"
