...
- 호스트의 승인 모델이 사용자별이다.
- 머신 범위는 여러 사용자가 같은 스캐너를 쓰는 시나리오인데,
  WinUSB가 동시 사용을 지원하지 않으므로 이득이 적다.

**단 드라이버 바인딩은 관리자 권한이 필요하다.** 설치는 사용자 범위,
드라이버는 관리자 — 이 분리를 UI에서 명확히 한다.
...
**Windows 설치가 macOS보다 단순하다.** 인터넷 연결이 필요 없고,
빌드가 없고, 관리자 권한이 (드라이버를 제외하면) 필요 없다.

대신 드라이버 바인딩이라는 새로운 복잡성이 생긴다.
